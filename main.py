...
def draw(win, images, player_car, computer_car, game_info):
    ...
    time_text = MAIN_FONT.render(f'Tempo {game_info.get_level_time()}', 1, (255,255,255))
...
def handle_collision(player_car, computer_car, game_info):
    ...
    if computer_finish_poi_collide != None:
        blit_text_center(WIN, MAIN_FONT, 'Você perdeu!')
        pygame.display.update()
...
while run:
    ...
    while not game_info.started:
        blit_text_center(WIN, MAIN_FONT, f'Precione alguma tecla p/ iniciar o nivel {game_info.level}!')
        pygame.display.update()
    ...
    if game_info.game_finished():
        blit_text_center(WIN, MAIN_FONT, "Você ganhou o jogo!")
        pygame.time.wait(5000)
        game_info.reset()
        player_car.reset()
        computer_car.reset()
